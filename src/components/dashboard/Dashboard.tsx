...
import { LeaderAnalyticsTab } from './LeaderAnalyticsTab';
...
export function Dashboard() {
  ...
  const navItems = [
    ...
    { id: 'leader-analytics', label: 'تحليلات الفريق', icon: BarChart3, visible: isLeader || isAdmin },
  ...
  const SidebarContent = ({ onNavigate }: { onNavigate?: () => void }) => (
    <div className="flex flex-col flex-1">
      ...
      {/* Current Team / Team Switcher */}
      <div className="p-3 border-b border-slate-200 dark:border-slate-700">
        <TeamSwitcher />
        {user?.currentTeam && (
          <div className="mt-2 flex items-center gap-2 text-xs">
            <span className="text-slate-500 dark:text-slate-400">كود الدعوة:</span>
            <code className="font-mono text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/30 px-2 py-0.5 rounded">
        ...
        <div className="flex-1 p-4 md:p-6 overflow-auto">
          {user?.currentTeam ? (
            <>
              ...
              {activeTab === 'leader-analytics' && (isLeader || isAdmin) && <LeaderAnalyticsTab />}
